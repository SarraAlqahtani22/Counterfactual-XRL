...
import wandb
wandb.init(name='Counterfactual - KL_Whitebox - all', project="counterfactual_based_attack")
...
LR_C = 0.02    # learning rate for critic 0.002
...
def train(arglist):
    with U.single_threaded_session():
        ...
        while True:
            ...
            KL_1 = sum(kl_div(MADDPG_next_action_n[1], MADDPG_next_action_n_atk[1])) + sum(kl_div(MADDPG_next_action_n_atk[1], MADDPG_next_action_n[1]))
            ...
            if train_step % (arglist.max_episode_len - 1) == 0:
                DDPG_rew += -100 * rew_n[1]
            # print("The reward is " + str(DDPG_rew))


            # DDPG_new_obs = np.append(new_obs_n[2], [attack_action_n[0], attack_action_n[1]])
