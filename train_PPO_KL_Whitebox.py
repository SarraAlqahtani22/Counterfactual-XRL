...
def train(arglist):
    with U.single_threaded_session() as marl_sess:
        ...
        attack_rate = 0.0  # attack threshold
        ...
        if loaded:
            print("loading-------------------------------------------------------------")
            saver_PPO.restore(marl_sess, './weights_PPO_PD_final/')
        ...
        while True:
            ...
            if terminal and (len(episode_rewards) % arglist.save_rate == 0):
                ...
                PPO_ep_rw = 0
                if not loaded:
                    # saver_PPO.save(marl_sess, './weights_PPO_CN_save/', global_step =len(episode_rewards))
                    saver_PPO.save(marl_sess, './weights_PPO_PD_save/')
